import { MentionAttach } from "../src/db-manager/types";
...
while (i <= text.length) {
    if (text[i] === '@') {
        tokens.push(text.slice(j, i));
        i += lengths[k] + 1;
        if (text[i] === ' ')  // @ + mention + ' ' (멘션하면 자동으로 뒤에 공백 붙음)
            i++;

        j = i;
        tokens.push(mentions[k++]);
    }
...
//         now += lengths[mentionIdx];
//         if (text[now + 1] === ' ')  // @ + mention + ' ' (멘션하면 자동으로 뒤에 공백 붙음)
//             now++;
